...
public class QueueConfigTest {
	...
	public void setUp() {
		...
		expectedQueueConfig = QueueConfig.builder().durable(false).autoDelete(false).exclusive(false)
				.deadLetterEnabled(false).build();
	...
	public void queueConfigWithNameAndValidationSuccessTest() {
		...
		assertThat(outputCapture.toString(),
				containsString(String.format("Queue configuration validated successfully for queue '%s'", queueName)));
	}
	...
	public void queueConfigWithoutNameAndValidationFailTest() {
		...
		assertThat(outputCapture.toString(),
				containsString("Invalid Queue Configuration : Name must be provided for a queue"));
	}
	...
	public void queueConfigWithOnlyQueueNameAndDefaultConfigurationAppliedTest() {
		...
		expectedQueueConfig = QueueConfig.builder().name(queueName).durable(false).autoDelete(false).exclusive(false)
				.deadLetterEnabled(false).build();
		assertThat(queueConfig, equalTo(expectedQueueConfig));
	}
	...
	public void queueConfigWithOnlyQueueNameAndNoDefaultConfigurationAppliedTest() {
		queueConfig = QueueConfig.builder().name(queueName).build();
		expectedQueueConfig = QueueConfig.builder().name(queueName).durable(null).autoDelete(null).exclusive(null)
				.deadLetterEnabled(null).build();
		assertThat(queueConfig, equalTo(expectedQueueConfig));
	}

	@Test
	public void queueConfigWithOnlyQueueNameAndFewDefaultConfigurationTest() {
		defaultQueueConfig = QueueConfig.builder().durable(true).autoDelete(true).exclusive(true)
				.deadLetterEnabled(true).argument("key1", "value1").build();

		queueConfig = QueueConfig.builder().name(queueName).build().applyDefaultConfig(defaultQueueConfig);

		expectedQueueConfig = QueueConfig.builder().name(queueName).durable(true).autoDelete(true).exclusive(true)
				.deadLetterEnabled(true).argument("key1", "value1").build();
	...
	public void queueConfigByOverridingFromDefaultConfigurationTest() {
		...
		queueConfig = QueueConfig.builder().name(queueName).durable(false).autoDelete(false)
				.argument("key1", "NEW_VALUE").argument("key2", "value2").build()
				.applyDefaultConfig(defaultQueueConfig);

		expectedQueueConfig = QueueConfig.builder().name(queueName).durable(false).autoDelete(false).exclusive(true)
				.deadLetterEnabled(true).argument("key1", "NEW_VALUE").argument("key2", "value2").build();
	...
	public void createQueueWithNoDeadLetterAndDefaultConfigurationAppliedTest() {
		...
		queueConfig = QueueConfig.builder().name(queueName).build();
		expectedQueueConfig = QueueConfig.builder().name(queueName).durable(false).autoDelete(false).exclusive(false)
				.deadLetterEnabled(false).arguments(new HashMap<>()).build();
	...
	public void createQueueWithFewDefaultConfigurationTest() {
		defaultQueueConfig = QueueConfig.builder().durable(true).autoDelete(true).exclusive(true)
				.deadLetterEnabled(false).argument("key1", "value1").build();

		queueConfig = QueueConfig.builder().name(queueName).durable(false).autoDelete(false).argument("key2", "value2")
				.argument("key1", "NEW_VALUE").build().applyDefaultConfig(defaultQueueConfig);

		expectedQueueConfig = QueueConfig.builder().name(queueName).durable(false).autoDelete(false).exclusive(true)
				.deadLetterEnabled(true).argument("key2", "value2").argument("key1", "NEW_VALUE").build();
	...
	public void createQueueWithDeadLetterAndDefaultDeadLetterConfig() {
		...
		deadLetterConfig = DeadLetterConfig.builder()
				.deadLetterExchange(ExchangeConfig.builder().name("dead-letter-exchange").build()).build();

		expectedQueueConfig = QueueConfig.builder().name(queueName).durable(false).autoDelete(false).exclusive(false)
				.deadLetterEnabled(true).argument("key1", "value1")
				.argument("x-dead-letter-exchange", "dead-letter-exchange")
				.argument("x-dead-letter-routing-key", queueName + DeadLetterConfig.DEFAULT_DEAD_LETTER_QUEUE_POSTFIX)
				.build();
	...
	public void createQueueWithDeadLetterAndDeadLetterConfig() {
		...
		deadLetterConfig = DeadLetterConfig.builder()
				.deadLetterExchange(ExchangeConfig.builder().name("dead-letter-exchange").build())
				.queuePostfix(".dlq-new").build();

		expectedQueueConfig = QueueConfig.builder().name(queueName).durable(false).autoDelete(false).exclusive(false)
				.deadLetterEnabled(true).argument("key1", "value1")
				.argument("x-dead-letter-exchange", "dead-letter-exchange")
				.argument("x-dead-letter-routing-key", queueName + ".dlq-new").build();
	...
	public void createDeadLetterQueueWithDefaultConfigurationAppliedTest() {
		...
		deadLetterConfig = DeadLetterConfig.builder()
				.deadLetterExchange(ExchangeConfig.builder().name("dead-letter-exchange").build())
				.queuePostfix(".dlq-new").build();
		expectedQueueConfig = QueueConfig.builder().name(queueName + ".dlq-new").durable(false).autoDelete(false)
				.exclusive(false).arguments(new HashMap<>()).build();
	...
	public void createDeadLetterQueueWithDefaultConfigurationPreAppliedTest() {
		...
		deadLetterConfig = DeadLetterConfig.builder()
				...
				.queuePostfix(".dlq-new").build();
		expectedQueueConfig = QueueConfig.builder().name(queueName + ".dlq-new").durable(false).autoDelete(false)
				.exclusive(false).deadLetterEnabled(true).arguments(new HashMap<>()).build();
		Queue queue = queueConfig.buildDeadLetterQueue(defaultQueueConfig, deadLetterConfig);
		assertQueue(queue, expectedQueueConfig);
	}
