...
public class DeadLetterConfigTest {
	...
	public void invalidDeadLetterConfigWithoutExchange() {
		deadLetterConfig = DeadLetterConfig.builder().deadLetterExchange(null).queuePostfix(".dlq").build();
		assertFalse(deadLetterConfig.validate());
	}

	@Test
	public void invalidDeadLetterConfigWithoutExchangeName() {
		deadLetterConfig = DeadLetterConfig.builder().deadLetterExchange(ExchangeConfig.builder().build())
				.queuePostfix(".dlq").build();
		assertFalse(deadLetterConfig.validate());
	}
}
