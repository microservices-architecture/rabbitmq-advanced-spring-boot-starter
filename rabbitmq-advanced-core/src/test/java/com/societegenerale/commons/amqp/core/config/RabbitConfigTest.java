...
public class RabbitConfigTest {
	...
	public void rabbitConfigWithValidExchangeAndQueueAndBindingTest() {
		rabbitConfig = RabbitConfig.builder().defaultExchange(createDefaultExchangeConfig())
				.defaultQueue(createDefaultQueueConfig()).deadLetterConfig(createValidDeadLetterConfig())
				.exchange(exchange, createExchangeConfig(exchange)).queue(queue, createQueueConfig(queue))
				.binding(binding, createBinding(exchange, queue, routingKey)).build();
	...
	public void rabbitConfigWithValidExchangeAndQueueAndBindingAndDeadLetterAndInvalidRequeueTest() {
		rabbitConfig = RabbitConfig.builder().defaultExchange(createDefaultExchangeConfig())
				.defaultQueue(createDefaultQueueConfig()).deadLetterConfig(createValidDeadLetterConfig())
				.reQueueConfig(createReQueueConfig(null, "requeue")).exchange(exchange, createExchangeConfig(exchange))
				.queue(queue, createQueueConfig(queue)).binding(binding, createBinding(exchange, queue, routingKey))
				.build();
		expectedException.expect(RabbitmqConfigurationException.class);
		rabbitConfig.validate();
	}
	...
	public void rabbitConfigWithInvalidExchangeAndValidQueueAndValidBindingTest() {
		rabbitConfig = RabbitConfig.builder().defaultExchange(createDefaultExchangeConfig())
				.defaultQueue(createDefaultQueueConfig()).deadLetterConfig(createValidDeadLetterConfig())
				.exchange(exchange, createExchangeConfig(null)).queue(queue, createQueueConfig(queue))
				.binding(binding, createBinding(exchange, queue, routingKey)).build();
		rabbitConfig.validate();
	}
	...
	public void rabbitConfigWithValidExchangeAndInvalidQueueAndValidBindingTest() {
		rabbitConfig = RabbitConfig.builder().defaultExchange(createDefaultExchangeConfig())
				.defaultQueue(createDefaultQueueConfig()).deadLetterConfig(createValidDeadLetterConfig())
				.exchange(exchange, createExchangeConfig(exchange)).queue(queue, createQueueConfig(null))
				.binding(binding, createBinding(exchange, queue, routingKey)).build();
		rabbitConfig.validate();
	}
	...
	public void rabbitConfigWithValidExchangeAndValidQueueAndInvalidBindingTest() {
		rabbitConfig = RabbitConfig.builder().defaultExchange(createDefaultExchangeConfig())
				.defaultQueue(createDefaultQueueConfig()).deadLetterConfig(createValidDeadLetterConfig())
				.exchange(exchange, createExchangeConfig(exchange)).queue(queue, createQueueConfig(queue))
				.binding(binding, BindingConfig.builder().queue(queue).routingKey(routingKey).build()).build();
		rabbitConfig.validate();
	}
	...
	public void rabbitConfigWithDeadLetterEnabledAndNullDeadLetterConfigTest() {
		rabbitConfig = RabbitConfig.builder().defaultExchange(createDefaultExchangeConfig())
				.defaultQueue(createDefaultQueueConfig()).deadLetterConfig(null)
				.exchange(exchange, createExchangeConfig(exchange)).queue(queue, createQueueConfig(queue))
				.binding(binding, createBinding(exchange, queue, routingKey)).build();
		rabbitConfig.validate();
	}
	...
	public void rabbitConfigWithNoDeadLetterEnabledAndNullDeadLetterConfigTest() {
		rabbitConfig = RabbitConfig.builder().defaultExchange(createDefaultExchangeConfig())
				...
				.deadLetterConfig(null).exchange(exchange, createExchangeConfig(exchange))
	...
	public void rabbitConfigWithNoDeadLetterEnabledAndNullDeadLetterExchangeTest() {
		rabbitConfig = RabbitConfig.builder().defaultExchange(createDefaultExchangeConfig())
				...
				.queue(queue,
						QueueConfig.builder().name(queue).build()
								.applyDefaultConfig(QueueConfig.builder().deadLetterEnabled(false).build()))
				.binding(binding, createBinding(exchange, queue, routingKey)).build();
		rabbitConfig.validate();
		assertThat(outputCapture.toString(), containsString("RabbitConfig Validation done successfully"));
	}
	...
	public void rabbitConfigWithNoDeadLetterEnabledForQueueAndNoDefaultDeadLetterExchangeTest() {
		rabbitConfig = RabbitConfig.builder().defaultExchange(createDefaultExchangeConfig()).defaultQueue(null)
				.deadLetterConfig(DeadLetterConfig.builder().deadLetterExchange(null).build())
				.exchange(exchange, createExchangeConfig(exchange))
	...
	public void rabbitConfigWithNoExchangeAndNoQueueAndNoBindingTest() {
		...
		rabbitConfig = RabbitConfig.builder().defaultExchange(null).defaultQueue(null).deadLetterConfig(null)
				.exchanges(new HashMap<>()).queues(new HashMap<>()).bindings(new HashMap<>()).build();
		rabbitConfig.validate();
		assertThat(outputCapture.toString(), containsString("RabbitConfig Validation done successfully"));
	}
	...
	private DeadLetterConfig createValidDeadLetterConfig() {
		return DeadLetterConfig.builder()
				.deadLetterExchange(ExchangeConfig.builder().name("dead-letter-exchange.dlx").build()).build();
	}

	private ReQueueConfig createReQueueConfig(String exchange, String queue) {
		return ReQueueConfig.builder().exchange(createExchangeConfig(exchange)).queue(createQueueConfig(queue))
				.routingKey("requeue.key").enabled(true).build();
	}
