...
public class DeadLetterConfig extends AbstractConfig {
	...
	public String createDeadLetterQueueName(String queueName) {
		return new StringBuilder().append(queueName).append(getDefaultConfig("DeadLetterConfig", "queuePostfix",
				queuePostfix, null, DEFAULT_DEAD_LETTER_QUEUE_POSTFIX)).toString();
	}
	...
	public boolean validate() {
		...
		if (deadLetterExchange != null && deadLetterExchange.validate()) {
			log.info("DeadLetterConfig configuration validated successfully for deadLetterExchange '{}'",
					deadLetterExchange);
			return true;
		}
		log.error(
