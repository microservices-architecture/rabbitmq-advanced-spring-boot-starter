...
public class RabbitConfig {
	...
	/**
	 * If 'true' : Auto configure the rabbitmq components on startup. Default value
	 * is 'true'
	 */
	@Builder.Default
	...
	/**
	 * Default Exchange configuration applies to all the missing configuration of
	 * each Exchange. This can be overridden by configuring at each exchange level.
	 */
	@NestedConfigurationProperty
	private ExchangeConfig defaultExchange;

	/**
	 * Default Queue configuration applies to all the missing configuration of each
	 * Queue. This can be overridden by configuring at each queue level.
	 */
	@NestedConfigurationProperty
	private QueueConfig defaultQueue;

	/**
	 * Dead Letter Configuration to configure dead letter exchange and queue
	 * postfix.
	 */
	@NestedConfigurationProperty
	...
	/**
	 * Info Headers can be used to add additional information to be added in each
	 * message headers
	 */
	@Singular
	...
	private boolean isDeadLetterEnabled() {
		if (defaultQueue != null && BooleanUtils.isTrue(defaultQueue.getDeadLetterEnabled())) {
			return true;
		} else {
			for (QueueConfig currentQueue : queues.values()) {
				return BooleanUtils.isTrue(currentQueue.getDeadLetterEnabled());
			}
			return false;
		}
	}
